...
4. **nomographic** — ArcadeDB database schemas and Flyway migrations (central server + local embedded)
...
│  └──────────────┘ └──────────────┘ └──────────────┘ └────────────┘ │
│  ┌──────────────────────────────────────────────────────────────┐   │
│  │ nomographic (ArcadeDB schemas — Flyway migrations)          │   │
...
| **nomographic** | SQL/Cypher/Java | ArcadeDB schemas & Flyway migrations; central fleet DB + local on-device embedded DB |
...
nomographic manages database schemas via Flyway migrations for two distinct ArcadeDB deployment targets:

| Path | Responsibility |
|------|---|
| `central/sql/` | Flyway migrations for the fleet-wide server database (vehicle registry, telemetry, users) |
| `central/flyway.toml` | Flyway config for the central server instance |
| `local/sql/` | Flyway migrations for the on-device embedded database (operational state, local intelligence) |
| `local/flyway.toml` | Flyway config for the local embedded instance |
...
- [ ] No credentials or secrets in migration scripts or Flyway configs
- [ ] Central and local migration versions are independent
- [ ] `flyway validate` passes for both central and local
...
- [ ] All tests passing (`cargo test`, `pytest`, `npx expo lint`, `flyway validate`)
...
- **Purpose:** DDL scripts and Flyway migrations for all ArcadeDB instances (central fleet server + local embedded per-device)
- **Stack:** ArcadeDB (document + graph), SQL/Cypher DDL, Flyway migrations, Java
- **Key Commands:** `flyway -configFiles=central/flyway.toml migrate`, `flyway -configFiles=local/flyway.toml migrate`
- **Structure:** `central/` for fleet server, `local/` for on-device embedded
- **Migration naming:** `V{N}__{description}.sql` (Flyway convention)
